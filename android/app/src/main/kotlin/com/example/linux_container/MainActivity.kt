...
class MainActivity : FlutterFragmentActivity() {
            ...
            .setMethodCallHandler { call, result ->
                when (call.method) {
                    "load" -> {
                        ...
                        Thread {
                            val err = InferenceEngine.load(applicationContext, path, gpu)
                    ...
                    "unload" -> { InferenceEngine.close(); result.success(true) }
                    "importModel" -> {
                    ...
                    "testFunctionCalling" -> {
                        ...
                        Thread {
                            val (err, text) = LiteRtEngine.testFunctionCalling(applicationContext, modelPath, useGpu)
                            runOnUiThread { if (err == null) result.success(text) else result.error("FNTEST", err, null) }
                        }.start()
                    }
                    "testGemma4ToolWithArgs" -> {
                        val modelPath = call.argument<String>("modelPath")
                        val useGpu = call.argument<Boolean>("useGpu") ?: true
                        if (modelPath == null) { result.error("ARG", "Falta modelPath", null); return@setMethodCallHandler }
                        Thread {
                            val (err, text) = LiteRtEngine.testGemma4ToolWithArgs(applicationContext, modelPath, useGpu)
                            runOnUiThread { if (err == null) result.success(text) else result.error("FNTEST5", err, null) }
                        }.start()
                    }
                    "testGemma4NativeToolCall" -> {
                        val modelPath = call.argument<String>("modelPath")
                        val useGpu = call.argument<Boolean>("useGpu") ?: true
                        if (modelPath == null) { result.error("ARG", "Falta modelPath", null); return@setMethodCallHandler }
                        Thread {
                            val (err, text) = LiteRtEngine.testGemma4NativeToolCall(applicationContext, modelPath, useGpu)
                            runOnUiThread { if (err == null) result.success(text) else result.error("FNTEST4", err, null) }
                        }.start()
                    }
                    ...
                    "testFunctionGemmaFlashlight" -> {
                        ...
                        if (modelPath == null) { result.error("ARG", "Falta modelPath", null); return@setMethodCallHandler }
                        Thread {
                            val (err, text) = LiteRtEngine.testFunctionGemmaFlashlight(applicationContext, modelPath, useGpu)
                            runOnUiThread { if (err == null) result.success(text) else result.error("FNTEST2", err, null) }
                        }.start()
                    }
                    "serverStart" -> {
                        ...
                        Thread {
                            var err: String? = null
                            if (path != null && !InferenceEngine.isLoaded)
                                err = InferenceEngine.load(applicationContext, path, gpu)
                            if (err == null && !InferenceEngine.isLoaded)
                    ...
                    "serverStatus" -> result.success(mapOf(
                        ...
                        "modelLoaded" to InferenceEngine.isLoaded,
                        "modelPath"   to (InferenceEngine.loadedPath ?: "")
                    ))
                    else -> result.notImplemented()
                }
            }
    ...
    private fun runGenerate(prompt: String, result: MethodChannel.Result) {
        ...
        var firstNs = 0L
        val err = InferenceEngine.generate(prompt) { token, done ->
            ...
            runOnUiThread {
                ...
                if (done) {
                    ...
                    val toks     = InferenceEngine.sizeInTokens(sb.toString())
