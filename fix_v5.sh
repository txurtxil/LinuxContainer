#!/bin/bash
# ============================================================================
# Linux Container v5.0 - Alpine static binaries via linker64
#
# PROBLEMA: linker64 NO puede cargar musl .so (PHDR error)
# SOLUCION: usar binarios ESTATICOS Alpine (busybox-static, apk-tools-static)
#            Los static musl binaries SI funcionan via linker64 (como proot-rs)
#
# Package: busybox-static-1.37.0-r31, apk-tools-static-3.0.6-r0
# Shell:   linker64 busybox.static sh -c "cmd"
# Package: linker64 apk.static update/add/del
# ============================================================================
set -e
cd "$(dirname "$0")"
...
echo -e "${BLUE}═══════════════════════════════════════════${NC}"
echo -e "${BLUE}  Linux Container v5.0 - Static Binaries${NC}"
echo -e "${BLUE}═══════════════════════════════════════════${NC}"

mkdir -p .backup
cp lib/services/proot_service.dart .backup/proot_service_v4.bak 2>/dev/null || true

echo -e "\n${YELLOW}[1/3] Escribiendo proot_service.dart v5.0...${NC}"
...
echo -e "  ${GREEN}✓${NC} proot_service.dart v5.0 ($(wc -l < lib/services/proot_service.dart) lineas)"

# Commit y push
echo -e "\n${YELLOW}[2/3] Commit y push...${NC}"
git add lib/services/proot_service.dart
git commit -m "v5.0: Alpine static binaries (busybox-static + apk-tools-static)

PROBLEMA DEFINITIVO:
- linker64 NO carga musl-linked shared libs (libc.musl) -> PHDR error
- linker64 SI carga binarios ESTATICOS musl (como proot-rs demo)

SOLUCION: usar Alpine static packages
1. busybox-static-1.37.0-r31 -> busybox.static (shell + utilidades)
2. apk-tools-static-3.0.6-r0 -> apk.static (package manager)
3. Ambos son ESTATICOS musl, funcionan via linker64
4. Se descargan como .apk (gzip+tar) desde mirror Alpine
5. Wrapper functions en shell para comandos interactivos

APKINDEX parsing implementado en Dart para resolver versiones" 2>/dev/null || true
...
echo -e "\n${GREEN}═══════════════════════════════════════════${NC}"
echo -e "${GREEN}  ✅ v5.0 COMPLETADO${NC}"
