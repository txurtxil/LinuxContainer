#!/bin/bash
# ============================================================================
# Linux Container App v3.0 - Apply fixes and push to GitHub
# ============================================================================
set -e
...
echo -e "${BLUE}═══════════════════════════════════════════${NC}"
echo -e "${BLUE}  Linux Container v3.0 - Aplicando fixes${NC}"
echo -e "${BLUE}═══════════════════════════════════════════${NC}"

# Backup original
echo -e "\n${YELLOW}[1/5] Respaldando archivos originales...${NC}"
mkdir -p .backup
cp lib/services/proot_service.dart .backup/proot_service.dart.bak
cp lib/screens/terminal_screen.dart .backup/terminal_screen.dart.bak
cp lib/services/terminal_service.dart .backup/terminal_service.dart.bak
echo -e "  ${GREEN}✓${NC} Respaldos en .backup/"

# Escribir proot_service.dart
echo -e "\n${YELLOW}[2/5] Escribiendo proot_service.dart...${NC}"
...
echo -e "  ${GREEN}✓${NC} proot_service.dart ($(wc -l < lib/services/proot_service.dart) lineas)"

# ─── 3. terminal_screen.dart ───
echo -e "\n${YELLOW}[3/5] Escribiendo terminal_screen.dart...${NC}"
...
echo -e "  ${GREEN}✓${NC} terminal_screen.dart actualizado ($(wc -l < lib/screens/terminal_screen.dart) lineas)"

# ─── 4. terminal_service.dart ───
echo -e "\n${YELLOW}[4/5] Escribiendo terminal_service.dart...${NC}"
...
# ─── 5. Commit y Push ───
echo -e "\n${YELLOW}[5/5] Commit y push a GitHub...${NC}"

git add lib/services/proot_service.dart lib/screens/terminal_screen.dart lib/services/terminal_service.dart
git commit -m "v3.0: linker64 direct execution, musl download fix, terminal wrapper functions

- Eliminado proot-rs (no funciona Android 15+ por seccomp)
- runCommand: linker64 + binario rootfs directo, shell syntax eliminada
- runShell: terminal interactiva con wrapper functions (bypass execve)
- _ensureMuslLib: descarga musl libc desde Alpine si falta
- apk --no-scripts para instalacion robusta
- Terminal con indicador de progreso y log siempre visible" 2>/dev/null || true
