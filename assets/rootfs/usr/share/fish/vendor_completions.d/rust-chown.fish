complete -c chown -l from -d 'change the owner and/or group of each file only if its current owner and/or group match those specified here. Either may be omitted, in which case a match is not required for the omitted attribute' -r
complete -c chown -l reference -d 'use RFILE\'s owner and group rather than specifying OWNER:GROUP values' -r -F
complete -c chown -l help -d 'Print help information.'
...
complete -c chown -l quiet -d 'suppress most error messages'
complete -c chown -s R -l recursive -d 'operate on files and directories recursively'
complete -c chown -s f -l silent
...
complete -c chown -s V -l version -d 'Print version'
